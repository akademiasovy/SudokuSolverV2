...
public class Sudoku extends Canvas implements Runnable {
    ...
    public Sudoku() {
        ...
        this.gameplan = new Gameplan(null);
                    ...
                    for (int row = 0; row < 9&&!set; row++) {
                        for (int column = 0; column < 9&&!set; column++) {
                            ...
                            if (r.contains(this.mouseInput.getX(), this.mouseInput.getY())) {
                                this.gameplan.getTile(row, column).nextVal();
                                set = true;
                                break;
                            }
                        }
                        ...
                        if (resetButton.contains(this.mouseInput.getX(), this.mouseInput.getY())) {
                            for (int row = 0; row < 9; row++) {
                                for (int column = 0; column < 9; column++) {
                                    this.gameplan.getTile(row,column).setValue(0);
                                    this.gameplan.getTile(row,column).setFree(true);
                                }
                            }
        ...
        for (int row = 0; row < 9; row++) {
            for (int column = 0; column < 9; column++) {
                if (!this.gameplan.getTile(row,column).isFree()) {
                    String text = String.valueOf(this.gameplan.getTile(row, column).getValue());
